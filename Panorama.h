...
Mat panorama_process(Mat img1, Mat img2)
{
	...
	printf("meilleure pondération: %f  image: %d\n", meilleure_ponderation, indice_meilleure_ponderation);
	//waitKey(0);


	/********* ICI , Etape 3bis. On check la différence de distance entre deux keypoints dans les deux images.***********/
	// On récupère un facteur d'échelle x et y
	// si ce facteur d'échelle est un peu grand, on relance la partie 1 ?!
	...
	if ( indice_meilleure_ponderation < 4)   offset_lignes = 0;
	else if (indice_meilleure_ponderation < 8) offset_lignes = 1;
	else if (indice_meilleure_ponderation < 12) offset_lignes = 2;
	else offset_lignes = 3;

	if (indice_meilleure_ponderation == 0 || indice_meilleure_ponderation == 4 || indice_meilleure_ponderation == 8 || indice_meilleure_ponderation == 12 ) offset_colonnes = 0;
	else if (indice_meilleure_ponderation == 1 || indice_meilleure_ponderation == 5 || indice_meilleure_ponderation == 9 || indice_meilleure_ponderation == 13 ) offset_colonnes = 1;
	else if (indice_meilleure_ponderation == 2 || indice_meilleure_ponderation == 6 || indice_meilleure_ponderation == 10 || indice_meilleure_ponderation == 14 ) offset_colonnes = 2;
	else  offset_colonnes = 3;
	...
	// faux
	int decalage_x =  decalage_x = -(512 - miniature_x) - image_x;
	...
	/*
	    for(int i = 0 ; i < -decalage_x; i++)
	    {
	        panorama_image = merge_process(img1, img2, -i,0);
	        imshow("Offset ", panorama_image);
	        waitKey(0);
	    }
	    for(int i = 0 ; i < -decalage_y; i++)
	    {
	        panorama_image = merge_process(img1, img2, 0,-i);
	        imshow("Offset ", panorama_image);
	        waitKey(0);
	    }*/
